...
## browser_MCP_win 原生输入桥

Windows 壳启动后同时监听 `127.0.0.1:38473`，供 `../browser_MCP_win/` 把已解析的
DOM 视口坐标交给 Rust/enigo 执行。扩展无需令牌即可直接连接；监听器不接受局域网
连接，并拒绝普通网页来源。

设置页可立即启停桥接。插件侧只做 DOM/ARIA/Shadow DOM 感知、遮挡检查和
坐标解析；`browser_action`、`browser_drag` 及 `browser_tab` 的状态变更通过真实系统
鼠标键盘完成。Rust 会先按标签标题与扩展上报的窗口边界找到并激活浏览器，再按
Chromium 渲染窗口的真实物理矩形换算页面坐标，从而扣除浏览器工具栏并兼容 Windows
DPI 与页面缩放。浏览器最小化、锁屏、桥接被停用、
目标跨越多个显示器或 Windows 拒绝切到前台时，桥接器会拒绝动作而不是盲点。

桥接执行前还会识别目标浏览器进程拥有、或 Win32 owner 链指向目标浏览器的
`#32770` 文件选择/系统信息窗口并将其取消；与目标浏览器无关的对话框不会被触碰。
网站权限与其他页面/浏览器弹窗由 `browser_MCP_win` 的 `contentSettings` 和 MAIN world
安全守卫提前阻止；漏网弹窗可由 `dismiss_dialog` 原生动作激活目标浏览器并发送
`Escape` 取消，插件会在快速页面请求超时后自动执行一次并重试。
