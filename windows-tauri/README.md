...
`device/windows/`（Electron 版）仍是稳定线，本目录是并行的实验线，二者互不影响。
...
所有修改直接在 `windows-tauri/src/` 及 `src-tauri/` 中进行。
...
```bat
run-windows-tauri.bat
build-windows-tauri.bat
```
...
npm install
npm run tauri dev      # 开发模式（前端热更新 + Rust 调试构建；自动 sync 共享代码）
