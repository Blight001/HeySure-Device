...
function effectiveAgentToolDefs() {
    return [
        {
            name: 'manage_card',
            description: '自动化卡片唯一入口（管理 + 执行合一）。action=rules 返回卡片步骤类型（9 种 type 及其字段、默认超时）与运行规则（失败重试、变量输入、需显式 save_cookies 步骤保存 Cookie 等）——写卡片前必须先调用，字段与步骤类型只能取自规范，不要凭空编造；action=list 列出所有已保存卡片的基本信息（id、名称、步骤数、保存时间、是否选中）；action=get 读取指定卡片完整 JSON；action=write 创建新卡片或用同一个 id 覆盖已有卡片（需 cardData，至少含 name/website/steps，写入前会按规范校验并拒绝非法步骤类型）；action=delete 删除卡片；action=run 在当前活动标签页执行卡片，可用 inputs 按变量键覆盖各 type 步骤的输入文本（默认用步骤 text），执行中通过 task:progress 及时反馈完整过程（每步开始/完成/重试/错误），最终返回结果（可能耗时数分钟；同一时间只能有一个 run；失败操作最多重试 3 次）。无论成功或失败，结果都含 execution 字段——完整执行明细：startedAt/finishedAt/durationMs 总耗时、stepsTotal/stepsExecuted/succeeded/failed/skipped/retries 统计，以及 steps[]（每步 stepIndex/stepName/status/attempts/durationMs 与 attemptDetails 逐次尝试的耗时和错误），可据此复盘每一步实际发生了什么、卡在哪一步、耗时多久。run 失败时另返回结构化现场：errorCode、失败步骤 stepIndex/stepName/selector、failureSnapshot（当前页 URL/标题 + 近似候选元素）与 context（已用到的变量值）；页面停留在失败现场——修复卡片（write）后用 action=run + start_step=stepIndex 并通过 inputs 回传已用到的变量值即可从失败步骤继续，不必从头执行。',
            input_schema: {
                type: 'object',
                properties: {
